...
def decimal_serializer(obj):
    if isinstance(obj, Decimal):
        # Convert Decimal to float or string (you can choose)
        return str(obj)  # or float(obj) if you prefer float
...
def collect_data(process_list, interval=1):
    ...
    return data, cpu_time1, cpu_time2

def calculate_percentages(data, cpu_time1, cpu_time2):
...
def main(pid=None, interval=1, lines=None, sort_key='total_cpu_pct'):
    ...
    d = collect_data(process_list, interval=interval)
    data = calculate_percentages(*d)
    ...
    try:
        for item in data:
            print(json.dumps(item, sort_keys=True, default=decimal_serializer, indent=2))
